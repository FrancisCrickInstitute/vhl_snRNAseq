...
```{r mon_umap, results = F, warning = F}
...
We can now cluster the cells. Monocle3 does not require a user-defined clustering resolution, as it can detect an optimum resolution automatically. Additionally, it groups the data into both high-resolution (cluster) and low-resolution (supercluster, a.k.a. partition) communities simultaneously.
...
Once cells have been clusters, we can isolate genes that are highly differentially expressed between the clusters.

```{r mon_cluster_markers, results = F, warning = F}
marker_test_res <- monocle3::top_markers(cds, group_cells_by = "cluster", 
                                         reference_cells = 1000, 
                                         speedglm.maxiter = 100)
```
    ...
    for(ct in names(avail_markers)) {
      ...
      if(n_ct_markers > 1) {
        knit <- knitr::knit_child(text = c(
          '',
          paste0('```{r ', paste0(chunk_name, "_module"), ', fig.dim = c(10,5)}'),
          'seu <- Seurat::AddModuleScore(seu, features = list(ct_markers), name = chunk_name)',
          'p <- final_clusters_umap + dittoSeq::dittoDimPlot(seu, paste0(chunk_name, "1")) + umap_void_theme'
          'p',
          '```',
          ''
      ...
      if(n_ct_markers < 50) {
        knit <- knitr::knit_child(text = c(
          '',
          paste0('```{r ', chunk_name, ', echo = F, fig.dim = get_fig_dims((n_ct_markers + 1), n_cols = 4, height_to_width_ratio = 1.2)}'),
