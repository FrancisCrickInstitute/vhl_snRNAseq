...
```{r groupings_vs_reductions, class.source = 'fold-hide', fig.dim = c(10, 40), fig.cap = "Plot of all Monocle3 dimensionality reductions, coloured by groupings"}
...
cds <- xfun::cache_rds({
  ...
  SummarizedExperiment::colData(cds)$cluster <- as.factor(monocle3::clusters(cds))
  SummarizedExperiment::colData(cds)$partition <- as.factor(monocle3::partitions(cds))
  cds
...
readr::write_tsv(top_cds_markers %>% dplyr::bind_rows(.id = "cell_grouping"), paste0(out$dea, "top_3_markers.tsv"))
```
...
p <- monocle3::plot_cells(cds, genes = avail_markers$HIF)
p
```
