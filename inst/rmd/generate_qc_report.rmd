...
```{r local_testing, include = F, eval = T}
local_testing = T
...
```{r small_test, include = F, eval = "local_testing" %in% ls()}
seu <- seu[sample(nrow(seu), size = as.integer(0.5*nrow(seu))), 
           sample(ncol(seu), size = as.integer(0.1*ncol(seu)))]
```
...
```{r nuclei_per_sample_plot, class.source = 'fold-hide', message = F, warning = F}
p <-
...
```{r filters_vs_distribution_vln, class.source = 'fold-hide', message = F, warning = F, fig.height = 10, fig.width = 10, fig.cap = "Plot of filters vs feature distribution"}
p <- 
...
```{r fail_criteria_proportions, class.source = 'fold-hide', fig.cap = "Plot of the proportions of filtered nuclei and their fail criteria for each sample"}
p <- 
  seu@misc$nucleus_filtering %>%
  dplyr::group_by(sample) %>%
...
```{r nucleus_scatter_with_filters, class.source = 'fold-hide', fig.width = 10, fig.cap = "Plot of filters against distribution of nuclei"}
p <- 
    plot_nucleus_scatter_with_filters(seu, "nCount_RNA", "percent_mito", ditto_colours, log_y = T) +
...
```{r hvg_scatter, class.source = 'fold-hide', warning = F, message = F, fig.cap = "Plot of HVGs"}
p <- 
  Seurat::LabelPoints(
...
```{r hvg_heatmap, class.source = 'fold-hide', fig.cap = "Heatmap of top 20 HVGs"}
p <- 
...
```{r pc1_vs_pc2, class.source = 'fold-hide', fig.cap = "Plot of PC1 vs PC2, coloured by sample"}
p <- 
...
```{r umap_split_by_sample, class.source = 'fold-hide', fig.width = 10, fig.height = dplyr::n_distinct(seu$nih_pid) * 1.5, fig.cap = "Plot of UMAP, split by patient"}
p <-
...
#### Important celltype markers (curated by Annika Fendler)

Various important stromal celltypes.

```{r avail_ctm, echo = F}
# subset celltype marker modules to only genes present in the dataset
ctm <- get_available_markers(seu, celltype_markers)
```

```{r plot_ct_markers, class.source = 'fold-hide', fig.dim = get_fig_dims(length(unlist(ctm)))}
p <- 
  plot_markers_on_umap(seu, ml = ctm)
p
```

#### HIF markers from Lombardi et al., 2022 (PMC9869179)

The Lombardi paper experimentally validated a list of HIF target genes. HIF activation is a principal ccRCC characteristic. 

```{r avail_hif, echo = F}
# subset HIF markers to only genes present in the dataset
hif <- get_available_markers(seu, hif_metagene)
```

Each nucleus is scored by the level of combined expression of the HIF metagene profile.

```{r hif_module_score}
seu <- Seurat::AddModuleScore(seu, hif_metagene, name = "HIF_metagene")
```

```{r plot_hif_metagene, class.source = 'fold-hide', fig.width = 10}
p <- 
  dittoSeq::dittoDimPlot(seu, "HIF_metagene1") +
  dittoSeq::dittoDimPlot(seu, "sample") +
  ggplot2::theme(legend.position = "none")
p
```

We can also inspect the expression of every constituent gene of the HIF metagene profile in all nuclei. 

```{r plot_hif_markers, class.source = 'fold-hide', fig.dim = get_fig_dims(length(unlist(hif)))}
p <- 
  plot_markers_on_umap(seu, ml = hif)
p
```

#### Kidney lineage markers from Young et al., 2019 (PMC6104812)

The Young paper used markers from different renal developmental lineages to do a comprehensive search for all resident celltypes. Authors curated markers from the literature for the following celltypes...

`r paste("*", paste0("**", names(kidney_lineage_markers), "**"), collapse = "  \n")`

The present dataset will be probed for evidence of these different celltypes.

```{r avail_kidney_lineage_markers, echo = F}
# subset kidney lineage marker modules to only genes present in the dataset
klm <- get_available_markers(seu, kidney_lineage_markers)
```

```{r plot_kidney_lineage_markers, echo = F, fig.dim = get_fig_dims(length(unlist(klm)))}
p <- 
  plot_markers_on_umap(seu, ml = klm)
p
```

```{r markers, fig.width = 10, echo = FALSE, message = FALSE, results = "asis"}
names(markers) %>%
  purrr::walk(function(study) {
    info <- stringr::str_split(study, "_")[[1]]
    if (length(info) == 3) {
      cat(paste0(" \n###[", info[3], " et al. (", info[2], 
               ")](https://pubmed.ncbi.nlm.nih.gov/", info[1], ") markers  \n"))
    } else {
      cat(paste0(" \n###", info, "\n"))
    }
    names(markers[[study]]) %>%
      purrr::walk(function(source) {
        cat(paste0("  \nMarkers sourced from ", source, ".  \n"))
        avail_markers <- get_available_markers(seu, markers[[study]][[source]])
        markers[[study]][[source]]
        p <-
          plot_markers_on_umap(seu, ml = avail_markers)
        p
        cat("  \n")
      })
  })
