...
agg_mat <- 
  monocle3::aggregate_gene_expression(
    cds, 
    gene_group_df %>% dplyr::filter(grepl("^all_", unique_module)), 
...
```{r modules_vs_partitions_heatmap, eval = multiple_partitions == T, echo = F, warning = F, message = F, fig.dim = c(10, 20), fig.cap = "Heatmap of all module scores for all Monocle3 partitions"}
agg_mat <- 
...
consensus_avail_markers <- 
  list(All = get_available_markers(cds, markers)$All)
...
for(study in names(consensus_avail_markers)) {
  ...
  for(source in names(consensus_avail_markers[[study]])) {
    ...
    #     
    #     annotations[[chunk_name_lvl]] <- 
    #       consensus_avail_markers[[study]][[source]] %>% 
    #       purrr::map(dplyr::as_tibble) %>% 
    #       dplyr::bind_rows(.id = "annotation") %>% 
    #       dplyr::mutate(unique_gene = paste(annotation, value) %>% janitor::make_clean_names())  %>% 
    #       tibble::column_to_rownames("unique_gene")
    #     annotation_colours[[chunk_name_lvl]] <- 
    #       list(annotation = c(dittoSeq::dittoColors()[1:dplyr::n_distinct(annotations[[chunk_name_lvl]]$annotation)]) %>%
    #              setNames(unique(annotations[[chunk_name_lvl]]$annotation)))
    ...
    #       monocle3::aggregate_gene_expression(cds[annotations[[chunk_name_lvl]]$value, ],
    #                                           cell_group_df = cell_group_df[[chunk_name_lvl]][, c("cell", "cell_group")])
    #     rownames(agg_mat[[chunk_name_lvl]]) <- rownames(annotations[[chunk_name_lvl]])
    #     
    #     knit_lvl_module_heatmap <- c(
    #       '',
    #       paste0('```{r ', chunk_name_lvl, '_module_heatmap, class.source = "fold-hide",  warning = F, message = F, fig.dim = c(10, max(length(unlist(consensus_avail_markers[[study]][[source]])) / 6, 3))}'),
    #       paste0('p <- pheatmap::pheatmap(agg_mat[["', chunk_name_lvl, '"]], scale = "row", cluster_rows = F, clustering_method = "ward.D2", annotation_row = annotations[["', chunk_name_lvl, '"]][, "annotation", drop = F], annotation_col = cell_group_df[["', chunk_name_lvl, '"]][, c("cell_group", "partition")] %>% dplyr::distinct() %>% tibble::column_to_rownames("cell_group"), labels_row = annotations[["', chunk_name_lvl, '"]]$value, annotation_colors = annotation_colours[["', chunk_name_lvl, '"]], annotation_names_row = F, gaps_row = which(annotations[["', chunk_name_lvl, '"]]$annotation != dplyr::lag(annotations[["', chunk_name_lvl, '"]]$annotation)) - 1, fontsize = 6)'),
    #       'p',
    #       '```',
    #       ''
    #       )
