...
```{r umap_vs_singler_annots, class.source = 'fold-hide', results = F, fig.width = 10, fig.height = 4, fig.cap = "Plot of UMAP clusters, coloured by SingleR annotation and labelled by the major SingleR annotation(s) per cluster"}
...
p <-
  ...
  umap_void_theme +
  dittoSeq::dittoDimPlot(seu, "lesion_type", size = 0.7) +
  umap_void_theme + 
  ggplot2::theme(legend.position = "bottom")
p
```
...
agg_mat <- monocle3::aggregate_gene_expression(cds, gene_group_df, cell_group_df[, c("cell", "cluster")])
...
```{r modules_vs_partitions_heatmap, echo = F, warning = F, message = F, fig.dim = c(10, 20), fig.cap = "Heatmap of all module scores for all Monocle3 partitions"}
# check if >1 partition
if (dplyr::n_distinct(cell_group_df$partition) > 1) {
  agg_mat <- monocle3::aggregate_gene_expression(cds, gene_group_df, cell_group_df[, c("cell", "partition")])
  agg_mat <- agg_mat[rowSums(is.na(agg_mat)) == 0, ] 
  annotations <- annotations[rownames(agg_mat),]
  p <- 
  pheatmap::pheatmap(agg_mat, scale = "row", clustering_method = "ward.D2",
