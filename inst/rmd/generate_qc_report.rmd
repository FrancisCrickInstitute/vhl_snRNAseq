...
```{r nuclei_per_sample_plot, echo = F, message = F, warnings = F}
p <-
...
```{r summary_statistics_plot, echo = F, message = F, warnings = F}
p <-
...
```{r filters_vs_distribution_vln, echo = F, message = F, warning = F}
p <- 
...
```{r sctransform, warning = F, message = F}
...
The dimensionality of the dataset to use for downstream analysis must be decided. This is the number of principal components believed to capture the majority of true biological signal in the dataset. Using too many PCs usually does not affect the result too much, while including too few PCs can be detrimental as meaningful biological variation may be dropped from downstream analysis. Therefore, it is better to be overgenerous when defining the dimensionality of the data than to underestimate.

The dimensionality can be decided by consulting the elbow plot and manually picking a value (argument `n_dims`) or it can be calculated using the `intrinsicDimensionality` package. 

#### `r if (!is.null(n_dims)) {"Manually"}`

`r if (!is.null(n_dims)) {paste0("Dimensionality of the data is **", n_dims, "** (set by argument n_dims).")}`
`r if (is.null(n_dims)) {"No value was provided for n_dims. Dimensionality will be calculated using the intrinsicDimensions package."}`
