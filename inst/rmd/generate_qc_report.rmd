...
```{r final_clusters, class.source = 'fold-hide', fig.width = 5, fig.cap = "Plot of final clusters"}
p <-
...
We can now cluster the cells. Monocle3 does not require a user-defined clustering resolution, as it can detect an optimum resolution automatically. Additionally, it groups the data into both high-resolution (cluster) and low-resolution (supercluster/partition) communities simultaneously.
    ...
    for(ct in names(avail_markers)) {
      ...
      if(n_ct_markers > 1) {
        knit <- knitr::knit_child(text = c(
          '',
          paste0('```{r ', paste0(chunk_name, "_module"), ', echo = F, fig.dim = get_fig_dims((n_ct_markers + 1), n_cols = 4, height_to_width_ratio = 1.2)}'),
