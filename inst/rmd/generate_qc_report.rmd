---
title: "snRNAseq QC report for ccRCC"
...
# chunk setup
cache_dir <- paste0(output_dir, "/cache/")
...
groupings <-
  ...
  { if (length(data_dir) > 1) c(., "experiment") else . }
```

# Run info {-}

```{r args, echo = F}
knitr::kable(tibble::tibble(argument = names(args), value = args %>% purrr::map(paste, collapse = ", ") %>% unlist %>% substr(1, 100)))
```

# Output info {-}

Output files will be saved to **`r output_dir`**.

Figures will be saved to **`r output_dir`qc_report_files/figure-html/**.

Intermediate objects will be cached to **`r cache_dir`**.

# Detect snRNA-seq platform

``` {r detect_platform}
input_files <- list.files(data_dir)
platform_files <- list(
...
Load snRNA-seq output as a Seurat object with no cut-offs, remove any missing samples, add sample metadata, add summary statistics, and optionally subset samples. Then filter nuclei per gene. 
...
seu <- xfun::cache_rds({
  ...
  if (platform == "10X Genomics") {
    ...
    dge_mat <- Seurat::Read10X(data_dir)
    ...
    seu <-
      ...
      Seurat::CreateSeuratObject(
        ...
        min_cells = min_n_nuclei_per_gene
      )
    
  }
  
  if (!is.null(sample)) {
    seu@meta.data$sample <- sample
  }
...
```{r summary_statistics_plot, class.source = 'fold-hide', eval = do_add_summary_stats == T, message = F, warning = F, fig.height = 15, fig.cap = "Plot of summary statistics by sample"}
p <-
  seu@meta.data %>%
  tibble::as_tibble() %>%
...
# save seu_integrated
saveRDS(seu, paste0(output_dir, "/seu_integrated.rds"))
```
...
```{r annotate_cell_cycle_phases, eval = do_cell_cycle_scoring == T}
...
dea_dir <- paste0(output_dir, "/dea/")
dir.create(dea_dir, showWarnings = F)
...
readr::write_tsv(top_cds_markers %>% dplyr::bind_rows(.id = "cell_grouping"), 
                 paste0(dea_dir, "top_3_markers.tsv"))
```
...
}, file = "cell_groupings_summary.rds", rerun = rerun)
