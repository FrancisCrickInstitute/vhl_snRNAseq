...
}, file = "seu_filtered.rds", dir = out$base)
```
...
}, file = "seu_transcformed.rds", dir = out$base)
```
...
seu <- xfun::cache_rds({
  ...
  seu
}, file = "seu_nonlin_dim_red.rds", dir = out$base)
```
...
seu <- xfun::cache_rds({
  ...
  seu
}, file = "seu_clustered.rds", dir = out$base)
```
...
}, file = "cds_nonlin_dim_red.rds", dir = out$base)
```
...
}, file = "cds_clustered.rds", dir = out$base) 
```
...
seu <- xfun::cache_rds({
  ...
  seu
}, file = "seu_auto_annotated.rds", dir = out$base)
```
...
for(study in names(markers)) {
  ...
  if (length(info) == 3) {
    author <- info[3]
    for_heading <- paste0(author, " et al. (", info[2], ")")
    for_text <-  paste0("[", for_heading, "](https://pubmed.ncbi.nlm.nih.gov/", info[1], ")")
  } else {
    author <- info
    for_heading <- for_text <- info
  }
  
  heading <- paste("###", for_heading, "markers\n")
  ...
  for(source in names(markers[[study]])) {
    ...
    seu <- Seurat::AddModuleScore(seu, features = avail_markers, name = )
    ...
    for(ct in names(avail_markers)) {
      ...
      if(n_ct_markers > 1) {
        knit <- knitr::knit_child(text = c(
          ...
          paste0('p <- final_clusters_umap + dittoSeq::dittoDimPlot(seu, paste0(chunk_name, "1"), size = 0.5, main = "', ct, ' module score") + umap_void_theme'),
          'p',
          '```',
          ''
      ...
      if(n_ct_markers < 50) {
        knit <- knitr::knit_child(text = c(
          ...
          'p <- plot_markers_on_umap(seu, ml = ct_markers, final_clusters_umap, umap_void_theme)',
          'p',
          '```',
          ''
        ), envir = environment(), quiet = TRUE)
        ...
        to_knit <- c(to_knit,
                     paste(n_ct_markers, "markers in this set - too many to print!\n"))
      }
    }
  }
}
...
```{r mon_cluster_markers, results = F, warning = F}
# marker_test_res <- monocle3::top_markers(cds, group_cells_by = "cluster", 
#                                          reference_cells = 1000)
cluster_markers <- top_markers_debugged(cds, group_cells_by = "cluster")
partition_markers <- top_markers_debugged(cds, group_cells_by = "partition")

# save top markers
readr::write_tsv(marker_test_res, paste0(out$base, "mon_top_markers.tsv"))
```

```{r mon_top_cluster_markers, class.source = 'fold-hide', fig.dim = c(10, 5), fig.cap = "Plot of the mean expression across cells and percentage of cells in each group that express the top 3 marker genes of each group"}
...
seu$cds_cluster <- cds@clusters$UMAP$clusters
dittoSeq::dittoDotPlot(seu, 
                       top_specific_marker_ids, 
                       group.by = "cds_cluster") +
