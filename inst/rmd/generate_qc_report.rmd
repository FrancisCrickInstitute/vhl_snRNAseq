...
args$parse_dir <- paste0(get_base_dir(), "/parse_pipeline/") 
...
```{r testing_subset, include = testing == T, eval = testing == T}
seu <- seu[sample(nrow(seu), size = as.integer(0.2*nrow(seu))), 
...
```{r filtering_stats, echo = F, eval = F}
...
```{r samples_vs_reductions, class.source = 'fold-hide', fig.dim = get_fig_dims(dplyr::n_distinct(seu$nih_pid), height_to_width_ratio = 1.25), fig.cap = "Plot of UMAP and tSNE, coloured by sample"}
p <-
...
```{r umap_split_by_sample, class.source = 'fold-hide', fig.dim = get_fig_dims(dplyr::n_distinct(seu$nih_pid)), fig.cap = "Plot of UMAP, split by patient"}
p <-
...
for(study in names(markers)) {
  info <- stringr::str_split(study, "_")[[1]]
      if (length(info) == 3) {
        heading <- paste0(
          "#### [", info[3], " et al. (", info[2], ")](https://pubmed.ncbi.nlm.nih.gov/", info[1], ") markers\n")
      } else {
        heading <- paste("####", info, "markers\n")
      }
  ...
  for(source in names(markers[[study]])) {
    ...
    for(ct in names(avail_markers)) {
      ...
      knit <- knitr::knit_child(text = c(
        '',
        '```{r echo = F, fig.dim = get_fig_dims(n_ct_markers, height_to_width_ratio = 0.8)}',
...
file.rename("output/", scevan_dir)

# add SCEVAN results to seu metadata
seu <- Seurat::AddMetaData(seu, metadata = results)

# save seu_scevan
saveRDS(seu, file = paste0(out$base, "/seu_scevan.rds"))
 
# ![SCEVAN copy number of each sample!](`r scevan_dir`/all_compareClonalCN.png){width = "1280"}
# ![SCEVAN tSNE vs CNA!](`r scevan_dir`/tsne_CNA.png){width = "1280"}
# ![SCEVAN tSNE vs classification!](`r scevan_dir`/tsne_scRNA.png){width = "1280"}
# ![SCEVAN OncoPrint!](`r scevan_dir`/OncoHeat.png){width = "1280"}
```
