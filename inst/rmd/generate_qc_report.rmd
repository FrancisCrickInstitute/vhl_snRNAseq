...
# testing: # args <- dget("out/221202_A01366_0326_AHHTTWDMXY/hg38/SHE5052A9_S101/all-well/DGE_filtered/unintegrated/20230226_223350/args_for_generate_qc_report.R") ; args$parse_dir <- "/Volumes/TracerX/working/VHL_GERMLINE/tidda/parse_pipeline/" ; out <- list(base = "out/221202_A01366_0326_AHHTTWDMXY/hg38/SHE5052A9_S101/all-well/DGE_filtered/unintegrated/20230226_223350/")
...
```{r summary_statistics_plot, echo = F, message = F, warning = F, fig.height = 10, fig.cap = "Plot of summary statistics by sample"}
p <-
...
```{r detected_doublets_plot, eval = remove_doublets == T, echo = F, fig.cap = "Plot of detected doublets in each sample (using scDblFinder)"}
p_dat <- 
...
p <- 
  ...
  ggplot2::geom_jitter(data = . %>% dplyr::filter(!is_doublet), height = 0,
                       ggplot2::aes(colour = "singlet"), size = 0.7) +
  ggplot2::geom_jitter(data = . %>% dplyr::filter(is_doublet), height = 0,
                       ggplot2::aes(colour = "doublet"), size = 0.7) +
...
```{r filters_vs_distribution_vln, echo = F, message = F, warning = F, fig.height = 10, fig.width = 10, fig.cap = "Plot of filters vs feature distribution"}
p <- 
...
```{r fail_criteria_proportions, echo = F, fig.cap = "Plot of the proportions of filtered nuclei and their fail criteria for each sample"}
p <- 
  seu@misc$nucleus_filtering %>%
  dplyr::group_by(sample) %>%
...
```{r nucleus_scatter_with_filters, echo = F, fig.width = 10, fig.cap = "Plot of filters against distribution of nuclei"}
p <- 
...
```{r hvg_scatter, echo = F, warning = F, message = F, fig.cap = "Plot of HVGs"}
p <- 
  Seurat::LabelPoints(
...
```{r hvg_heatmap, echo = F, fig.cap = "Heatmap of top 20 HVGs"}
p <- 
...
Samples will **`r if (do_integration == T) {"not"}`** be integrated.
...
Variables to regress out of the `SCTransform` residuals  (set by argument `vars_to_regress`) **`r paste(vars_to_regress, collapse = "** and **")`**. These variables are prevented from contributing much to the PCA during dimensionality reduction, and thus confounding the analysis due to irrelevant variation.
...
```{r pc1_vs_pc2, echo = F, fig.cap = "Plot of PC1 vs PC2, coloured by sample"}
p <- 
  dittoSeq::dittoDimPlot(seu, "sample", reduction.use = "pca", raster = F)
p
```

```{r pca_dim, echo = F, fig.cap = "Plot of cells and genes sorted by their PC scores for PC1 and PC2"}
p <- 
  Seurat::DimHeatmap(
    seu,
    dims = 1:2,
...
```{r top_genes_pc1_pc2, echo = F, fig.cap = "Plot of top genes associated with PC1 and PC2"}
p <- 
...
```{r elbow_plot, echo = F, fig.cap = "Elbow plot"}
p <-
...
```{r samples_vs_reductions, echo = F, fig.width = 10, fig.height = 6, fig.cap = "Plot of UMAP and tSNE, coloured by sample"}
p <-
...
```{r umap_split_by_sample, echo = F, fig.width = 10, fig.height = 6, fig.cap = "Plot of UMAP, split by sample"}
p <-
  dittoSeq::dittoDimPlot(seu, "nih_pid", reduction.use = "umap", raster = F, legend.show = F, split.by = "sample", split.ncol = 3)
p
```

```{r groupings_vs_reductions, echo = F, fig.dim = c(10, 30), fig.cap = "Plot of all dimensionality reductions, coloured by groupings"}
# amend groupings
...
```{r resolutions_vs_reductions, echo = F, fig.dim = c(10, 30), fig.cap = "Plot of all reductions, coloured by clusters"}
...
```{r clusters_per_sample, echo = F, fig.cap = "Plots of clusters per sample and samples per cluster"}
p <-
...
```{r singler_annots_heatmap, echo = F, fig.width = 10, fig.cap = "Heatmap of celltype annotation scores from SingleR"}
p <-
...
```{r singler_annots_delta, echo = F, results = F, warning = F, message = F, fig.cap = "Delta distribution of celltype annotations from SingleR"}
p <-
...
```{r umap_vs_singler_annots, echo = F, results = F, fig.width = 10, fig.height = 6, fig.cap = "Plot of UMAP clusters, coloured by SingleR annotation and labelled by the major SingleR annotation(s) per cluster"}
...
```{r singler_annots_bar, echo = F, fig.cap = "Plot of SingleR annotation composition of samples"}
p <-
...
```{r singler_annots_alluvial, echo = F, fig.width = 10, fig.cap = "Alluvial plot of assignment paths of nuclei from samples to clusters to SingleR annotations, coloured by cluster"}
p <-
  seu@meta.data %>%
...
The Lombardi paper experimentally validated a list of HIF target genes. HIF activation is a principal ccRCC. 
...
**`r paste(paste("*", names(kidney_lineage_markers)), collapse = "\n")`** 

The present dataset will be probed for evidence of these different celltypes.

```{r present_kidney_lineage_markers, include = F}
# subset kidney lineage marker modules to only genes present in the dataset
klm <- kidney_lineage_markers %>% purrr::map(intersect, rownames(seu))
klm <- Filter(function(x) length(x) > 0, klm)
```

```{r plot_kidney_lineage_markers}
p <- list()
names(klm) %>%
  purrr::walk(function(lineage) {
    ...
    p[[lineage]] <<- 
      ...
      top = lineage,
      ncol = 3,
      nrow = ceiling(length(klm[[lineage]])/3)
    )
    p[[lineage]]
  })
```
