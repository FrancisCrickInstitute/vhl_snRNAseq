...
out <- list(base = paste0(get_base_dir(), "/vhl/out/test/"))
    ...
    for(ct in names(avail_markers)) {
      ...
      if(n_ct_markers < 50) {
        knit <- knitr::knit_child(text = c(
          '',
          paste0('```{r ', chunk_name, ', echo = F, fig.dim = get_fig_dims(n_ct_markers, height_to_width_ratio = 0.8)}'),
...
```{r run_scevan, message = F, warning = F, eval = T}
