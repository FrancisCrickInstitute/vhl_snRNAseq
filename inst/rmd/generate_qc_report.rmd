...
![Barcode Rank!](`r paste(parse_dir, "analysis", experiment, genome, sublibrary, "all-well/figures/fig_tscp_cell_cutoff.png", sep = "/")`){width = "1280"}
...
```{r plot_markers, eval = T, echo = FALSE, results = 'asis'}
to_knit <- c()
for(study in names(markers)) {
  ...
  for(source in names(markers[[study]])) {
    ...
    for(ct in names(avail_markers)) {
      ...
      rlang::warn(chunk_name)
      if (grepl("\\W", chunk_name)) rlang::warn(sprintf("Illegal characters used in '%s'", chunk_name))
      
      knit <- knitr::knit_child(text = c(
        '',
        paste0('```{r ', chunk_name, 
               ', echo = F, fig.dim = get_fig_dims(n_ct_markers, height_to_width_ratio = 0.8)}'),
...
```{r scevan_output, eval = F}
knitr::include_graphics(
  paste0(
    scevan_dir,
    c("all_compareClonalCN.png",
      "tsne_CNA.png",
      "tsne_scRNA.png",
      "OncoHeat.png"
    )
