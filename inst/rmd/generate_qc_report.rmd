...
```{r groupings_vs_reductions, class.source = 'fold-hide', fig.dim = c(10, 30), fig.cap = "Plot of all Monocle3 dimensionality reductions, coloured by groupings"}
...
cds <- xfun::cache_rds({
  SummarizedExperiment::colData(cds)$singler_annot <- cds_singler %>%
        dplyr::as_tibble() %>%
        dplyr::group_by(labels) %>%
        dplyr::transmute(singler_annot = replace(labels, dplyr::n() < 10, "none")) %>%
        dplyr::pull(singler_annot)
  cds
}, file = "cds_auto_annotated.rds", rerun = rerun)
```

```{r singler_annots_heatmap, class.source = 'fold-hide', fig.width = 10, fig.cap = "Heatmap of celltype annotation scores from SingleR"}
annotation_col <- data.frame(cluster = SummarizedExperiment::colData(cds)$cluster,
                             partition = SummarizedExperiment::colData(cds)$partition)
...
p <-
  SingleR::plotScoreHeatmap(
    cbind(cds_singler, SummarizedExperiment::colData(cds)$partition), 
...
p <-
  SingleR::plotDeltaDistribution(cds_singler)
p
```

```{r umap_vs_singler_annots, class.source = 'fold-hide', results = F, fig.width = 10, fig.height = 5, fig.cap = "Plot of UMAP clusters, coloured by SingleR annotation and labelled by the major SingleR annotation(s) per cluster"}
...
# add celltype annotation cluster labels for plotting
SummarizedExperiment::colData(cds)$singler_annot_cluster_label <- 
  get_singler_annot_label(cds, "cluster")

# add celltype annotation partition labels for plotting
SummarizedExperiment::colData(cds)$singler_annot_partition_label <-
  get_singler_annot_label(cds, "partition")

# plot
p <-
  dittoSeq::dittoDimPlot(cds, "singler_annot", main = "singler annots, cluster-level", size = 0.7) +
  ggrepel::geom_label_repel(
    data = dplyr::as_tibble(SummarizedExperiment::colData(cds)) %>%
      dplyr::right_join(get_centroids(cds, "UMAP", "cluster"), by = "cluster") %>%
      dplyr::distinct(cluster, singler_annot_cluster_label, x, y),
    ggplot2::aes(x, y, label = singler_annot_cluster_label),
    min.segment.length = 0,
    size = 2, fill = 'white', alpha = 0.8, seed = 1234, label.size = NA
  ) +
  umap_void_theme +
  ggplot2::theme(legend.position = "bottom") +
  dittoSeq::dittoDimPlot(cds, "singler_annot", main = "singler annots, partition-level", size = 0.7) +
  ggrepel::geom_label_repel(
    data = dplyr::as_tibble(SummarizedExperiment::colData(cds)) %>%
      dplyr::right_join(get_centroids(cds, "UMAP", "partition"), by = "partition") %>%
      dplyr::distinct(partition, singler_annot_partition_label, x, y),
    ggplot2::aes(x, y, label = singler_annot_partition_label),
    min.segment.length = 0,
    size = 2, fill = 'white', alpha = 0.8, seed = 1234, label.size = NA
  ) +
  umap_void_theme +
  dittoSeq::dittoDimPlot(cds, "lesion_type", size = 0.7) +
  umap_void_theme + 
  ggplot2::theme(legend.position = "bottom")
p
```
...
p <-
  dplyr::as_tibble(SummarizedExperiment::colData(cds)) %>%
  dplyr::group_by(sample, partition, cluster, singler_annot) %>%
  dplyr::count() %>%
  dplyr::ungroup() %>%
  ...
  dplyr::mutate(x = dplyr::case_when(x == 1 ~ "sample",
                                     x == 2 ~ "partition",
                                     x == 3 ~ "cluster",
                                     x == 4 ~ "singler_annotation") %>%
                  factor(levels = c("sample", "partition", "cluster", "singler_annotation"))) %>%
  ...
  ggforce::geom_parallel_sets_labels(angle = 0, 
                                     nudge_x = c(
                                       ...
                                       rep(0.4, dplyr::n_distinct(SummarizedExperiment::colData(cds)$singler_annot))
                                     )) +
  ggplot2::theme_void() +
  ggplot2::theme(legend.position = "none",
...
agg_mat <- 
  monocle3::aggregate_gene_expression(cds, gene_group_df, cell_group_df[, c("cell", "cluster")])
agg_mat <- agg_mat[rowSums(is.na(agg_mat)) == 0, ] 
row_annotations_clust <- row_annotations[rownames(agg_mat), ]
p <- 
  pheatmap::pheatmap(agg_mat, scale = "row", clustering_method = "ward.D2",
                     labels_row = row_annotations$module,
                     annotation_row = row_annotations_clust[, c("source", "study")],
...
```{r modules_vs_partitions_heatmap, eval = multiple_partitions == T, warning = F, message = F, fig.dim = c(10, 20), fig.cap = "Heatmap of all module scores for all Monocle3 partitions"}
agg_mat <- 
  monocle3::aggregate_gene_expression(cds, gene_group_df, cell_group_df[, c("cell", "partition")])
agg_mat <- agg_mat[rowSums(is.na(agg_mat)) == 0, ] 
row_annotations_part <- row_annotations[rownames(agg_mat), ]
p <- 
  pheatmap::pheatmap(agg_mat, scale = "row", clustering_method = "ward.D2",
                     labels_row = row_annotations$module,
                     annotation_row = row_annotations_part[, c("source", "study")],
                     annotation_colors = colours,
                     fontsize = 8)
p
```
...
for(study in names(avail_markers)) {
  ...
  for(source in names(avail_markers[[study]])) {
    ...
    for(lvl in lvls) {
      ...
      chunk_name_lvl <- paste(chunk_name, lvl) %>% janitor::make_clean_names()
      
      if (dplyr::n_distinct(SummarizedExperiment::colData(cds)[, lvl]) > 1) {
        
        annotations[[chunk_name_lvl]] <- avail_markers[[study]][[source]] %>% purrr::map(dplyr::as_tibble) %>% dplyr::bind_rows(.id = "annotation") %>% dplyr::group_by(value) %>% dplyr::summarise(annotation = paste(annotation, collapse = ", ") %>% substr(1, 100)) %>% dplyr::arrange(annotation) %>% tibble::column_to_rownames("value")
        annotation_colours[[chunk_name_lvl]] <- list(annotation = c(dittoSeq::dittoColors()[1:dplyr::n_distinct(annotations[[chunk_name_lvl]]$annotation)]) %>% setNames(unique(annotations[[chunk_name_lvl]]$annotation)))
        cell_group_df[[chunk_name_lvl]] <- tibble::tibble(cell = row.names(SummarizedExperiment::colData(cds)), cell_group = SummarizedExperiment::colData(cds)[, lvl])
        agg_mat[[chunk_name_lvl]] <- monocle3::aggregate_gene_expression(cds[rownames(annotations[[chunk_name_lvl]])], cell_group_df = cell_group_df)
        longest_annot[[chunk_name_lvl]] <- annotations[[chunk_name_lvl]]$annotation %>% nchar() %>% max()
        
        knit_lvl_module_heatmap <- c(
          '',
          paste0('```{r ', chunk_name_lvl, '_module_heatmap, class.source = "fold-hide",  warning = F, message = F, fig.dim = c(10, length(unlist(avail_markers[[study]][[source]])) / 6)}'),
          paste0('p <- pheatmap::pheatmap(agg_mat[["', chunk_name_lvl, '"]], scale = "row", cluster_rows = F, clustering_method = "ward.D2", annotation_row = annotations[["', chunk_name_lvl, '"]], annotation_colors = annotation_colours[["', chunk_name_lvl, '"]], annotation_names_row = F, gaps_row = which(annotations[["', chunk_name_lvl, '"]]$annotation != dplyr::lag(annotations[["', chunk_name_lvl, '"]]$annotation)) - 1, fontsize = 6)'),
          'p',
          '```',
          ''
          )
      } else { 
        knit_lvl_module_heatmap <- paste0("Only one ", lvl, ", so cannot produce a heatmap!\n") 
      }
...
`r if (is.null(final_cluster_annotations)) { "Final celltype annotations of clusters have not been given. Please run the package again and pass celltype labels to the final_cluster_annotations argument to proceed with downstream analysis." }`

```{r exit_if_no_cts, include = is.null(final_cluster_annotations), eval = is.null(final_cluster_annotations)}
# truncate document if no final celltype annotations have been provided
knitr::knit_exit()
```

```{r final_cluster_annotations, echo = F}
# get final cluster annotations
final_cluster_annotations_df <-
  dplyr::left_join(
    tibble::as_tibble(SummarizedExperiment::colData(cds)),
    tibble::enframe(final_cluster_annotations,
                    name = "cluster",
                    value = "final_cluster_annotation"),
    by = "cluster"
  )
# encode in the colData
SummarizedExperiment::colData(cds)$final_cluster_annotation <-
  final_cluster_annotations_df$final_cluster_annotation
```

```{r fca_kable, echo = F}
final_cluster_annotations_df %>%
  dplyr::group_by(cluster, final_cluster_annotation) %>%
  dplyr::count() %>%
  knitr::kable()
```

```{r final_cluster_annotations_dimplot, class.source = 'fold-hide', fig.dim = c(10, 4.5), fig.cap = "Plot of Monocle3 UMAP, coloured by cluster and by final cluster annotation"}
p <- 
  final_umap[["cluster"]] + 
  dittoSeq::dittoDimPlot(cds, "final_cluster_annotation")
p
```
...
`r if ("endothelial" %ni% SummarizedExperiment::colData(cds)$final_cluster_annotation) { "inferCNV requires an endothelial cluster as a baseline for CNV analysis, but the final celltype annotations of clusters (final_celltype_annotations argument) do not include an 'endothelial' cluster. Please run the package again with an 'endothelial' cluster specified in order to run inferCNV." }`

```{r exit_if_no_endothelial, include = "endothelial" %ni% SummarizedExperiment::colData(cds)$final_cluster_annotation, eval = "endothelial" %ni% SummarizedExperiment::colData(cds)$final_cluster_annotation}
# truncate document if no endothelial celltype annotations have been provided
knitr::knit_exit()
```

We annotate endothelial cells as matched normal and use this to infer copy number variants in the tumour population. 
...
SummarizedExperiment::colData(cds)[, "final_cluster_annotation", drop = F] %>%
