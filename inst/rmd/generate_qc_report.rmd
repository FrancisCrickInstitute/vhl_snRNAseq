...
}, file = "seu_transcformed.rds", rerun = rerun)
```
...
## `Monocle3` clustering pipeline

The `Monocle3` package also offers an unsupervised clustering function. This function also separates cells into superclusters, called partitions. These represent higher order communities within the dataset, allowing the different resolutions to coexist for downstream analysis.
...
First, we must convert the filtered `Seurat` object to Monocle3's proprietary `cell_data_set` object and fill in the gene names.

```{r seu_to_cds, results = F, warning = F}
cds <- xfun::cache_rds({
  cds <- SeuratWrappers::as.cell_data_set(seu)
  SummarizedExperiment::rowData(cds)$gene_short_name <- row.names(SummarizedExperiment::rowData(cds))
  cds
...
Then, we preprocess the data again. This involves normalisation and principal component analysis. We use the previously decided dimensionality of the data (**`r final_n_dims`**).
...
p <-
  gridExtra::marrangeGrob(
    grobs = groupings_vs_reductions,
    nrow = length(avail_groupings),
    ncol = length(cds@reduce_dim_aux),
    layout_matrix = matrix(
      1:length(groupings_vs_reductions),
      length(avail_groupings),
...
```{r mon_cluster_markers, eval = T, results = F, warning = F}
...
cluster_markers <- xfun::cache_rds({
  top_markers_develop(cds, group_cells_by = "cluster")
}, file = "cds_cluster_markers.rds", rerun = rerun)
partition_markers <- xfun::cache_rds({
  top_markers_develop(cds, group_cells_by = "partition")
}, file = "cds_partition_markers.rds", rerun = rerun)

# get top 3 markers per cluster
top_specific_markers <- cluster_markers %>%
  dplyr::as_tibble() %>%
  dplyr::filter(fraction_expressing >= 0.10) %>%
  dplyr::group_by(cell_group) %>%
  dplyr::top_n(3, pseudo_R2) %>%
  dplyr::arrange(desc(cell_group))
top_specific_marker_ids <- unique(top_specific_markers %>% dplyr::pull(gene_id))
```

```{r mon_top_cluster_markers, eval = T, class.source = 'fold-hide', fig.dim = c(10, length(top_specific_marker_ids) / 4), fig.cap = "Plot of the mean expression across cells and percentage of cells in each group that express the top 3 marker genes of each group"}
...
```{r all_modules_heatmap, echo = FALSE, warning = F, message = F, fig.dim = c(10, 20), fig.cap = "Heatmap of all module scores for all Monocle3 clusters"}
gene_group_df <- 
  markers %>%
  ...
  dplyr::mutate(chunk_name = paste(gsub(".*\\_", "", study), source, module)) %>%
  dplyr::select(value, chunk_name)
cell_group_df <- 
  tibble::tibble(cell = row.names(SummarizedExperiment::colData(cds)), cell_group = monocle3::clusters(cds))
agg_mat <- 
...
for(sc_type in names(sc_objects)) {
  ...
  for(study in names(markers)) {
    ...
    for(source in names(markers[[study]])) {
      avail_markers <- get_available_markers(sc_objects[[sc_type]], markers[[study]][[source]])
      ...
      for(ct in names(avail_markers)) {
        ...
        if (sc_type == "seu") {
          knit <- 
            plot_seu_markers(chunk_name,
                             ct,
                             n_ct_markers,
                             seu)
        } else if (sc_type == "cds") {
          knit <-
            plot_cds_markers(chunk_name)
        }
        knit <- knitr::knit_child(text = knit, envir = environment(), quiet = TRUE)
        to_knit <- c(to_knit, knit)
      }
