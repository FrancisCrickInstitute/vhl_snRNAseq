...
rds_list_df <-
  tibble::tibble(
    V2 = system(paste0("ls ", out_dir, "N*/cache*/seu_pca*"), T)) %>%
  dplyr::mutate(V1 = V2 %>% gsub(".*unintegrated\\/", "", .) %>% gsub("\\/cache.*", "", .))
out_path <- paste0(out_dir, "tumour_specific_markers/")
tumor_ct <- "malignant"
idents <-
...
tumor_vs_rest_DE_fun <- function(sobj,tumor_ct,sample){
  ...
  DE_genes <- FindMarkers(sobj, ident.1=tumor_ct, logfc.threshold = 0)
...
pairwise_DE_fun <- function(sobj,tumor_ct,sample){
  ...
  for (ct in non_tumor_ct) {
    DE_tmp <- FindMarkers(sobj, ident.1 =tumor_ct, ident.2 = ct, logfc.threshold = 0)
...
for (sample_id in samples) {
  ...
  Idents(sobj) <- idents[colnames(cds), "partition_annot"]
  ...
  #step1 tumor vs all the rest cell types
